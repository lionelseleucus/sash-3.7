...
makeArgs(const char * cmd, int * retArgc, const char *** retArgv)
{
	...
	{	
		...
		while (*cp)
		{
			ch = *cp++;
   			...
   			/*
   		 	 * Check if the dollar has been already detected.
   	 		 *
   	 		 */
   			if (strncmp (cp,"$",1) == 0)
   			{
   				dollarAlreadyDetected = TRUE;
   			}
			...
			if (ch == '\\')
			{
				 ...
				 * is a wildcard.
				 */
				if (isWildCard(ch) && dollarAlreadyDetected == FALSE)
		 ...
		 */
		if (quotedWildCards && unquotedWildCards && dollarAlreadyDetected == FALSE)
		{
			fprintf(stderr,
		 ...
		 */
		if (unquotedWildCards && dollarAlreadyDetected == FALSE)
		{
			/*
	...
	*retArgc = argCount;
	...
	processingArgs(argCount, argTable);
...
/* 
 * Remove character specified in c
 *
 */
